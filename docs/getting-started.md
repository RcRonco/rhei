...
async fn main() -> anyhow::Result<()> {
    let readings = vec![
        Reading { sensor_id: "sensor-1".into(), value: 23.5 },
        Reading { sensor_id: "sensor-2".into(), value: 18.0 },
        Reading { sensor_id: "sensor-1".into(), value: 24.1 },
    ];

    let graph = DataflowGraph::new();
    graph
        .source(VecSource::new(readings))
        .filter_fn(|r| r.value > 20.0)
...
Run it with `cargo run`. Two readings pass the filter and are printed; `sensor-2` is dropped.
...
fn build(graph: &DataflowGraph) {
    let raw = graph.source(VecSource::new(vec![Raw { line: "a".into() }]));
    let parsed = raw.map(|r| Parsed { sensor_id: r.line.to_string() });
    parsed.sink(PrintSink::<Parsed>::new());
}
```

Closures passed to `map`, `filter_fn`, and `key_by` receive a **zero-copy view**, not an owned value. For `Reading` above, the generated view is:
...
`filter` is a separate method taking a column expression (`rhei::col("value").gt(rhei::lit_f64(20.0))`) that is evaluated as an Arrow kernel over the whole batch. Use `filter_fn` for closures.
...
fn build(graph: &DataflowGraph) {
    graph
        ...
        .filter_fn(|r| r.value > 0.0)   // runs where the data already is
        .key_by(|r| r.sensor_id.to_string()) // redistributes across workers
        .sink(PrintSink::<Reading>::new());
}
```
...
impl StreamFunction for CountEvents {
    ...
    async fn process(
        ...
        for view in &input {
            ...
            let count = state.get(&sensor_id).await?.unwrap_or(0) + 1;
            state.put(&sensor_id, &count)?;
            builder.append(Counted { sensor_id, count });
        }
...
fn build(graph: &DataflowGraph) {
    let window = TumblingWindow::new(
        60, // window size, in the same unit as the time field
        |view: ReadingView<'_>| view.sensor_id.to_string(),
        |view: ReadingView<'_>| view.timestamp,
        |acc: &mut f64, view: ReadingView<'_>| *acc += view.value,
    ...
    graph
        ...
        .key_by(|r| r.sensor_id.to_string())
        .operator("avg_temp", window)
        .sink(PrintSink::<WindowResult>::new());
}
```

`ReadingView<'_>` is generated by `#[derive(RheiSchema)]` and is in scope automatically.
...
fn validate(value: f64) -> anyhow::Result<()> {
    if value < 0.0 {
        let err = PipelineError::bad_data("negative sensor value");
...
impl Sink for CollectSink {
    ...
    async fn write_batch(&mut self, batch: RheiBuffer<Reading>) -> anyhow::Result<()> {
        ...
        for view in &batch {
            out.push(view.sensor_id.to_string());
        }
        Ok(())
    }
}
...
async fn main() -> anyhow::Result<()> {
    ...
    graph
        .source(VecSource::new(vec![
            Reading { sensor_id: "a".into(), value: 25.0 },
            Reading { sensor_id: "b".into(), value: 5.0 },
        ]))
        .filter_fn(|r| r.value > 20.0)
