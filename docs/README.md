...
| How do I build a window? | `Window::tumbling(n).key(..).time(..).accumulate(..).finish(..).build()`, or the positional `::new(...)`. [operators.md](operators.md#building-a-window) |
| Can I avoid stringly-typed column names in `filter`? | Yes — `T::col().field()` is generated by the derive. [operators.md](operators.md#typed-columns) |
